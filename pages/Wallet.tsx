                        ...
                        <div className="space-y-4">
                            {transactions.length === 0 ? (
                                ...
                                transactions.slice(0, 5).map((tx: any) => (
                                    <div key={tx.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5 transition-colors border-b border-white/5 last:border-0">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-full ${tx.type === 'DEPOSIT' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-blue-500/10 text-blue-500'}`}>
                                                {tx.type === 'DEPOSIT' ? <DollarSign size={16} /> : <Download size={16} />}
                                            </div>
                                            <div>
                                                <p className="text-gray-200 text-sm font-medium">
                                                    {tx.type === 'DEPOSIT' ? 'Coin Deposit' : tx.type === 'PURCHASE' ? 'Item Purchase' : tx.type}
                                                </p>
