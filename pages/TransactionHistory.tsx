...
const TransactionHistory: React.FC<TransactionHistoryProps> = ({ setPage }) => {
  ...
  const isEligibleForRefund = (tx: Transaction) => {
    ...
    if (tx.isUsed) return false;

    const txDate = new Date(tx.date);
    const diffTime = Math.abs(CURRENT_DATE.getTime() - txDate.getTime());
  ...
  const confirmRefund = () => {
    ...
    setTransactions(prev => prev.map(t => 
      t.id === selectedTx.id ? { ...t, status: 'refunded' } : t
    ));
